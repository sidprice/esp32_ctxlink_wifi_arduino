...
void wifi_get_net_info(void) {
  ...
  package_data(message, sizeof(network_connection_info_s),
               PROTOCOL_PACKET_TYPE_NETWORK_INFO);
  //
...
void task_wifi(void *pvParameters) {
  ...
  //
  wifi_comms_queue =
      xQueueCreate(wifi_comms_queue_length,
  ...
  WiFi.onEvent(
      onWiFiEvent,
      WiFiEvent_t::ARDUINO_EVENT_MAX); // Register the Wi-Fi event handler
    ...
    if (wifi_status != previous_status) {
      ...
      case WL_CONNECTED: {
        ...
        if (gdb_task_handle == NULL) {
          ...
          xTaskCreate(task_wifi_server, "GDB Server", 4096,
                      (void *)&gdb_server_params, 1, &gdb_task_handle);
        } else {
      ...
      //
      result =
          xQueueReceive(wifi_comms_queue, &message, wifi_state_poll_period);
      if (result == pdTRUE) {
        size_t data_length;
        size_t packet_size;
        ...
        //
        network_connection_info_s *conn_info =
            (network_connection_info_s *)packet_data;
        ...
        if (wifi_status == WL_CONNECTED) {
          ...
          //
          if (strcmp(ssid, conn_info->network_ssid) != 0 ||
              strcmp(password, conn_info->pass_phrase) != 0) {
