			...
			for (size_t i = 0; i < trans_ctx.size; ++i) {
				...
				if (errs[i] == ESP_OK) {
					...
					if (err != ESP_OK) {
						...
						ESP_LOGD(TAG, "transaction complete: %d bits (%d bytes) received", rtrans->trans_len,
							num_received_bytes);
					}
				} else {
...
public:
	...
	/// @param data6
	/// @param data7
	bool begin(uint8_t spi_bus, int sck, int ss, int data0, int data1, int data2, int data3, int data4, int data5,
		int data6, int data7)
	{
		this->ctx.if_cfg.spics_io_num = ss;
		this->ctx.bus_cfg.sclk_io_num = sck;
	...
	bool queue(uint32_t flags, const uint8_t *tx_buf, uint8_t *rx_buf, size_t size)
	{
		...
		if (this->transactions.size() >= this->ctx.if_cfg.queue_size) {
			ESP_LOGW(TAG, "failed to queue transaction: queue is full - only %u transactions can be queued at once",
				this->ctx.if_cfg.queue_size);
			return false;
		}
	...
	void setDMAChannel(spi_common_dma_t dma_chan)
	{
		if ((dma_chan == SPI_DMA_CH1) || (dma_chan == SPI_DMA_CH2) || (dma_chan == SPI_DMA_CH_AUTO)) {
			...
			ESP_LOGW(TAG, "invalid dma channel %d: make sure to select SPI_DMA_CH1, SPI_DMA_CH2 or SPI_DMA_CH_AUTO",
				dma_chan);
		}
	}
#endif
	...
	{
		this->ctx.if_cfg.post_setup_cb = post_setup_cb;
	}
	...
	{
		this->ctx.if_cfg.post_trans_cb = post_trans_cb;
	}
...
private:
	...
	bool initialize(const uint8_t spi_bus)
	{
		...
		int ret = xTaskCreatePinnedToCore(spi_slave_task, task_name.c_str(), SPI_SLAVE_TASK_STASCK_SIZE,
			static_cast<void *>(&this->ctx), SPI_SLAVE_TASK_PRIORITY, &this->spi_task_handle, 1);
		if (ret != pdPASS) {
