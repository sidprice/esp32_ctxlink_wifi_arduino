 ...
 */
bool configure_server(in_port_t *port, int *server_fd,
  ...
  *server_fd = socket(AF_INET, SOCK_STREAM, 0);
  ...
  if (listen(*server_fd, 5) <
      0) { // TODO: Test for single client operation, set to 1 or 0?
...
void task_wifi_server(void *pvParameters) {
  ...
  server_queue = xQueueCreate(
      server_queue_length,
      sizeof(uint8_t *)); // Create the queue for the GDB server task
  server_params->server_queue =
      server_queue; // Save the queue to the server task parameters
  socklen_t addr_len = sizeof(client_addr);

  in_port_t port =
      (in_port_t)server_params->port; // Recover the port number for this task
  //
  ...
  while (true) {
    ...
    while (true) {
      ...
      server_params->client_fd =
          client_fd; // Save the client file descriptor for the client task
      //
      // Start a task to handle this new client
      //
      xTaskCreate(task_client, "ClientTask", 4096, (void *)server_params, 5,
                  NULL);
      //
      ...
      while (true) {
        ...
        if (result == pdTRUE) {
          ...
          case PROTOCOL_PACKET_TYPE_COMMAND: {
            protocol_packet_command_s *command_packet =
                (protocol_packet_command_s *)packet_data;
            if (command_packet->command ==
                PROTOCOL_PACKET_TYPE_CMD_SHUTDOWN_GDB_SERVER) {
              MON_NL("Close Client/Server Sockets");
              close(client_fd);
              client_fd = -1;
              close(server_fd);
              server_fd = -1;
            } else if (command_packet->command ==
                       PROTOCOL_PACKET_TYPE_CMD_START_GDB_SERVER) {
